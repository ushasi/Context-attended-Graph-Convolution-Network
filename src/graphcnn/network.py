...
#-----------------------------------------------------
    def make_node_attention_layer(self, no_filters, name=None):
        print_ext("Attention layer")
        ...
        self.current_V = make_node_attention_layer(self.current_V, no_filters)
        #self.current_V = tf.math.sigmoid(self.current_V)
      
        print_ext("Shape of V at output:",self.current_V.get_shape())


        #self.attended_features = self.current_V
        return self.current_V, self.current_mask
#-----------------------------------------------------
    def make_edge_attention_layer(self, no_filters, name=None):
        print_ext("Attention layer")
        print_ext("Shape of V:",self.current_V.get_shape())
        print_ext("no of nodes in output:",self.current_V.get_shape()[1].value)
        print_ext("no of features in output:",no_filters)
        
        self.current_A = make_edge_attention_layer(self.current_A, no_filters)
        #self.current_V = tf.math.sigmoid(self.current_V)
      
        print_ext("Shape of A at output:",self.current_A.get_shape())


        #self.attended_edges = self.current_A
        return self.current_A, self.current_mask
#-------------------------------------------------------------------        
    ...
    def make_embedding_layer(self, no_filters, name=None, with_bn=True, with_act_func=True):
        ...
        print_ext("no of nodes in output:",self.current_V.get_shape()[1].value)
        print_ext("no of features in output:",no_filters)
        #with tf.variable_scope(name, default_name='Embeds') as scope:
    ...
    def make_fc_layer(self, no_filters, name=None, with_bn=False, with_act_func=True):
        ...
        print_ext("Shape of V at output:",self.current_V.get_shape())
	#print ('###################')
	#self.current_V = tf.nn.l2_normalize(self.current_V,dim=None)
	#print self.current_V.get_shape()
        if no_filters == 256: #earlier 512
            self.fc_features = self.current_V
            self.fc_features = tf.nn.relu(self.fc_features)
        return self.current_V
